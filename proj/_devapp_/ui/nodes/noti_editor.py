    ...
    def save_current_form(self):
        ...
        if type_str == "telegram":
            ...
            if not token or not chatid:
                QMessageBox.warning(self, "입력 오류", "Telegram API 토큰과 채팅 ID는 필수 입력 항목입니다.")
                if not token:
            ...
            if self.current_noti_key in self.noti_items and isinstance(self.noti_items[self.current_noti_key], TelegramNoti):
                noti_item = self.noti_items[self.current_noti_key]
                # 기존 항목 업데이트
                noti_item.name = name
                noti_item.type = type_str
                noti_item.message_title = message_title
                noti_item.acc_server = acc_server
                noti_item.acc_nickname = acc_nickname
                noti_item.comment = comment
                noti_item.zone = zone
                noti_item.repeat_min = repeat_min
                noti_item.enable = enable
                noti_item.token = token
                noti_item.chatid = chatid
                noti_item.baseurl = baseurl
            else:
                # 새 항목 생성
                noti_item = TelegramNoti()
                noti_item.name = name
                noti_item.type = type_str
                noti_item.message_title = message_title
                noti_item.acc_server = acc_server
                ...
                noti_item.chatid = chatid
                noti_item.baseurl = baseurl
                self.noti_items[self.current_noti_key] = noti_item
        else:  # Discord
            webhooks = self.webhooks_edit.text()
            
            # 필수 필드 검증
            if not webhooks:
                QMessageBox.warning(self, "입력 오류", "Discord Webhook URL은 필수 입력 항목입니다.")
                self.webhooks_edit.setFocus()
                return False
            
            # 이미 있으면 업데이트, 없으면 새로 생성
            if self.current_noti_key in self.noti_items and isinstance(self.noti_items[self.current_noti_key], DiscordNoti):
                noti_item = self.noti_items[self.current_noti_key]
                # 기존 항목 업데이트
                noti_item.name = name
                noti_item.type = type_str
