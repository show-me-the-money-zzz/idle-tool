...
STORE_FILE = STORE_DIR / "imageareas.json"
...
def AddItem(key, data, issave: True):
    ...
    print(f"store.AddItem({key}, {data})")

def GetItem(key, default=None):
    """텍스트 영역 항목 조회"""
    return ImageAreas.get(key, default)

def GetAllItems():
    """모든 텍스트 영역 항목 조회"""
    return ImageAreas.copy()

def DeleteItem(key):
    """텍스트 영역 항목 삭제"""
    if key in ImageAreas:
        del ImageAreas[key]
        return True
    return False
...
def Save():
    ...
    with open(STORE_FILE, "w", encoding="utf-8") as f:
        json.dump(ImageAreas, f, indent=2, ensure_ascii=False)
    return True
    ...
    if os.path.exists(STORE_FILE):
        try:
            ...
            if False == getattr(sys, 'frozen', False):#utils.system.DEVAPP
                print(f"store.Load(): {" / ".join(ImageAreas)}")
            return True
        except Exception as e:
            print(f"이미지 영역 데이터 로드 오류: {e}")
            ImageAreas = {}
    return False
...
def initialize():
    ...
    success = Load()
    if not success:
        print("이미지 영역 데이터를 로드할 수 없습니다. 새 데이터 저장소를 생성합니다.")
